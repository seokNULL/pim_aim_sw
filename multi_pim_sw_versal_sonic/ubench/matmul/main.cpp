...
static void parse_opt(int argc, char **argv) {
  ...
  for (int i = optind, j = 0; i < argc; ++i, ++j) {
    switch (j) {
      ...
      case 4: fpga_id = (size_t) atoi(argv[i]); break;
      default: break;
    }
  }
...
void matmul_pim(short *A, short *B, short *C, pim_args *pim_code, size_t M, size_t N, size_t K, int fpga_id) {
  ...
  // pim_code->srcB_va = (uint64_t)&A[0];	// modified by KJI
  // pim_code->srcA_va = (uint64_t)&B[0];	// modified by KJI
...
void matmul_decoupled_pim(short *A, short *B, short *C, pim_args *pim_code, size_t M, size_t N, size_t K, int fpga_id){
  int A_ROW_PAD;
  int num_of_reg = 16;
...
int main(int argc, char **argv) {
  ...
  /* Allocate and initialize matrices on CPU and PIM*/
    ...
    int srcA_size = p_size * q_size; // modified by KJI
    // int srcA_size = q_size * r_size; // modified by KJI
    ...
    short *ansC_pim = (short *)pim_malloc(dstC_size*sizeof(short), fpga_id);
    ...
    //   if (i%16 == 0) temp1 += 0.0005;
    //   srcB[i] = temp1;
    // }
    /* test for manual input end */
  ...
  // all data movement cost can be removed.
    ...
    switch (device) {
      ...
      case 2: matmul_decoupled_pim(srcA_pim, srcB_pim, dstC_pim, pim_code, p_size, r_size, q_size, fpga_id); break;
    }
    elapsed_time_iter += get_current_time();
...
//    printf("%.4f ns\n", elapsed_time_iter*1000000000);
  ...
  printf("\nResult check w/o. transpose!\n");
  for(int i=0; i<dstC_size; i++){ 
      printf("idx[%4d] ", i);
      printf("0x%x ", dstC_pim[i]);
      printf("\n");   
  }
  ...
  switch (device) {
    ...
    case 2: matmul_decoupled_pim_finalize(srcA_pim, srcB_pim, dstC_pim, ansC_pim, p_size, r_size, q_size); break;
  }
  ...
  if (print_matrix) {
    ...
    for (int i = 0; i < M; ++i) {
      for (int j = 0; j < N; ++j) { printf("%+.3f ", short_to_float(ansC_pim[i * N + j])); }
      printf("\n");
    }
  }
  ...
  if (validation) {
    ...
    switch (device) {
      ...
      case 2: check_mat_mul(ansC_pim, C_ans, p_size, r_size, q_size); break;
    }  
  }
