...
void check_mat_mul(short* result_pim0, short* result_pim1, float *C_ans, int M, int N, int K) {
  // xdma transfer to cpu buffer and check correctness
...
void copy_mat_input_each(float *src, short *dst0, short *dst1, int size){
  for(size_t i=0; i<size; i++){
    if(i<size/2){
      float tmp  = src[i];
      short tmp0 = float_to_short(tmp);
      dst0[i] = tmp0; // replace with xdma data copy
    }
    else{
      float tmp  = src[i];
      short tmp0 = float_to_short(tmp);
