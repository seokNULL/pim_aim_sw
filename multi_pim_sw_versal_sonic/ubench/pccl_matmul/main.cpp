...
void matmul_pccl(short *A, short *B, short *C, pim_args *pim_code, size_t M, size_t N, size_t K, int fpga_id) {
  const int src_id = 0;  // temporary hard coding
  // pim_args** pim_codes = (pim_args**)malloc(sizeof(pim_args*)*totalDevNum);

  pim_args* pim_codes[totalDevNum];
  short* pim_arrA[totalDevNum];
  short* pim_arrB[totalDevNum];
  short* pim_arrC[totalDevNum];

  for(int i=0; i<totalDevNum; i++) {
    pim_codes[i] = (pim_args *)malloc(sizeof(pim_args)); 
    pim_codes[i]->srcA_va = (uint64_t)(short*)pim_malloc(M*K*sizeof(short)/2, i);
    pim_codes[i]->srcB_va = (uint64_t)(short*)pim_malloc(K*N*sizeof(short), i);
    pim_codes[i]->dstC_va = (uint64_t)(short*)pim_malloc(M*N*sizeof(short)/2, i);
    pim_codes[i]->p_size = M/2;
    pim_codes[i]->q_size = K;
    pim_codes[i]->r_size = N;
    pim_arrA[i] = (short*)pim_codes[i]->srcA_va; // Array of matA destination addresses
    pim_arrB[i] = (short*)pim_codes[i]->srcB_va; // Array of matB destination addresses
    pim_arrC[i] = (short*)pim_codes[i]->dstC_va; // Array of matB destination addresses
  }
  pimScatter(pim_arrA, A, src_id, M*K*sizeof(short));
  pimBcast(pim_arrB, B, src_id, K*N*sizeof(short));

  double elapsed_time_iter = -get_current_time();
  pccl_matmul(pim_codes);
  elapsed_time_iter += get_current_time();
  printf("%.4f\n", elapsed_time_iter*1000);

  // for (int i = 0; i < M*N/2; ++i) { printf("%+.3f\n",short_to_float(pim_arrC[1][i])); }
        // for(int i = 0; i < M*N/2; i++) printf("a[%d]: %hu\n", i, pim_arrC[1][i]);
  pimGather(C, pim_arrC, src_id, M*N*sizeof(short));
  
        // for(int i = M*N/2; i < M*N; i++) printf("b[%d]: %hu\n", i, C[i]);
  // gather 구현은 p2p 수정 후에 다시
}
...
int main(int argc, char **argv) {
  ...
  /* Allocate and initialize matrices on CPU and PIM*/
    ...
    printf("srcA_pim: %llx\n", VA2PA(((uint64_t)&srcA_pim[0])));
    printf("srcB_pim: %llx\n", VA2PA(((uint64_t)&srcB_pim[0])));
    printf("dstC_pim: %llx\n", VA2PA(((uint64_t)&dstC_pim[0])));
    pim_args *pim_code;
  ...
  // all data movement cost can be removed.
    ...
    switch (device) {
      ...
      case 3: matmul_pccl(srcA_pim, srcB_pim, dstC_pim, pim_code, p_size, r_size, q_size, fpga_id); break;
    }
    elapsed_time_iter += get_current_time();
...
//    printf("%.4f ns\n", elapsed_time_iter*1000000000);
  ...
  if (print_matrix) {
    ...
    // printf("MATRIX C_ans:\n");
    // print_mat(C_ans, M, N); 
    printf("MATRIX C_ans\tPIM:\n");
    for (int i = 0; i < dstC_size/2; ++i) { printf("%+.3f\t%+.3f\n", C_ans[i], short_to_float(dstC_pim[i])); }
    printf("\n\n");
    for (int i = dstC_size/2; i < dstC_size; ++i) { printf("%+.3f\t%+.3f\n", C_ans[i], short_to_float(dstC_pim[i])); }
    // for (int i = 0; i < dstC_size; ++i) { printf("%+.3f\t%+.3f\n", C_ans[i], short_to_float(dstC_pim[i])); }
    printf("\n");
  }
  ...
  if (validation) {
    ...
    switch (device) {
      ...
      case 3: check_mat_mul(dstC_pim, C_ans, p_size, r_size, q_size); break;
    }  
  }
  ...
  pim_free(dstC_pim, fpga_id);  // pccl용 pim_free 추가 필요
