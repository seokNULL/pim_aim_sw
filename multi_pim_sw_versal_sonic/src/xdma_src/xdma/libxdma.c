...
static int irq_msix_user_setup(struct xdma_dev *xdev)
{
	...
	for (i = 0; i < xdev->user_max; i++, j++) {
		...
		u32 vector = xdev->entry[j].vector;
#endif
		rv = request_irq(vector, xdma_user_irq, IRQF_SHARED, xdev->mod_name,
				 &xdev->user_irq[i]);
		pr_info("user irq: %d\n", vector);	
		if (rv) {
...
/* xdma_desc_set_high0() - Fill a descriptor with the transfer details in p2p transfer
 *
 * @desc pointer to descriptor to be filled
 * @addr other pim memory physical address
 * @ep_addr end point address
 * @len number of bytes, must be a (non-negative) multiple of 4.
 * @dir, dma direction
 * is the end point address. If zero, vice versa.
 ...
 * Does not modify the next pointer
 */
static void xdma_desc_set_high0(struct xdma_desc *desc, dma_addr_t rc_bus_addr,
			  u64 ep_addr, int len, int dir)
{
	/* transfer length */
	...
	if (dir == DMA_TO_DEVICE) {
		/* read from root complex memory (source address) */
		desc->src_addr_lo = cpu_to_le32(PCI_DMA_L(rc_bus_addr));
		desc->src_addr_hi = cpu_to_le32(0x00000000UL);
		/* write to end point address (destination address) */
		desc->dst_addr_lo = cpu_to_le32(PCI_DMA_L(ep_addr));
		desc->dst_addr_hi = cpu_to_le32(PCI_DMA_H(ep_addr));
	} else {
			  ...
			  struct xdma_transfer *transfer)
{
	...
	engine->prev_cpu = get_cpu(); // get current processor number and disable kernel preemption
...
static int transfer_build(struct xdma_engine *engine,
			...
			unsigned int desc_max, int write)
{
	...
	for (; i < desc_max; i++, j++, sdesc++) {
		...
		// if p2p, another fill the high 32bit of source address with 0
		if (write == 2) 
			xdma_desc_set_high0(xfer->desc_virt + j, sdesc->addr, req->ep_addr,
					      		sdesc->len, xfer->dir);
		else
...
static int transfer_init(struct xdma_engine *engine,
			struct xdma_request_cb *req, struct xdma_transfer *xfer, int write)
{
...
/////////////////////////////////
// if p2p transfer, modify descriptor source address
	transfer_build(engine, req, xfer, desc_max, write);
//////
...
ssize_t xdma_xfer_aperture(struct xdma_engine *engine, int write, u64 ep_addr,
			...
			bool dma_mapped, int timeout_ms)
{
	...
	// unsigned int maxlen = desc_blen_max;
	unsigned int sg_max;
	unsigned int tlen = 0;
	u64 ep_addr_max = ep_addr + aperture - 1;
	// u64 ep_addr_max = 0x208FFFFFFFF;
	ssize_t done = 0;
	int i, rv = 0;
	...
	while (req->offset < req->total_len) {
		...
		for (sg = req->sg; i < sg_max && desc_idx < desc_max;
			...
			while (tlen) {
				...
				if (write == 2)
					xdma_desc_set_high0(engine->desc + desc_idx, addr,
						ep_addr, len, dir);
				else		
...
ssize_t xdma_xfer_submit(void *dev_hndl, int channel, int write, u64 ep_addr,
			 struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	...
	struct scatterlist *sg = sgt->sgl;
	int nents;
	enum dma_data_direction dir = write > 0 ? DMA_TO_DEVICE : DMA_FROM_DEVICE;
	struct xdma_request_cb *req = NULL;
	...
	while (nents) {
		...
		// modified
		/* build transfer */
		rv = transfer_init(engine, req, &req->tfer[0], write);
				...
				bool dma_mapped, int timeout_ms)
{
	...
	while (nents) {
		...
		xfer = &req->tfer[tfer_idx];
		/* build transfer */
		rv = transfer_init(engine, req, xfer, write);
		if (rv < 0) {
