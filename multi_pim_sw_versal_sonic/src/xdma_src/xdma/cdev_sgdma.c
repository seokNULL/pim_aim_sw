...
static void char_sgdma_unmap_user_buf(struct xdma_io_cb *cb, int write);

u64 va_to_ka(u64 vaddr) // TODO: split this function to translation-tailored device?
{
    pgd_t *pgd;
    pud_t *pud;
    pmd_t *pmd;
...
/*====== modified ======*/
	if (write == 2) {
		uint64_t kaddr = va_to_ka(buf);
		// printk("pim malloc kernel virtual address %llx\n", kaddr);
		// printk("pim malloc kernel virt_to_bus address %llx\n", virt_to_bus(kaddr));
		for (i = 0; i < pages_nr; i++) {
			if (kaddr) {
				cb->pages[i] = virt_to_page(kaddr);
				kaddr += XDMA_PAGE_SIZE;
			} else {
				pr_err("unable to make the page list\n");
				goto err_out;
			}
		}
		rv = pages_nr;
		...
		rv = get_user_pages_fast((unsigned long)buf, pages_nr, 1/* write */,		// prob // reasoning
					cb->pages);
	}
/*======================*/
	...
	for (i = 0; i < pages_nr; i++, sg = sg_next(sg)) {
		...
		// printk("check iteration #: %d\n", i);

		flush_dcache_page(cb->pages[i]);
		// printk("check flush #: %d, nbytes: %d, offset: %d\n", i, nbytes, offset);
		sg_set_page(sg, cb->pages[i], nbytes, offset);
		// printk("check set page #: %d\n", i);
...
static int ioctl_do_p2p_dma(struct file *file, unsigned long arg)
{
	struct xdma_aperture_ioctl io;
	int rv;
	...
	rv = copy_from_user(&io, (struct xdma_aperture_ioctl __user *)arg,
				sizeof(struct xdma_aperture_ioctl));
	if (rv < 0) {
		dbg_tfr("failed to copy from user space 0x%lx\n", arg);
		return -EINVAL;
	}
	// loff_t *pos;
	// pos = &io.ep_addr;
	return char_sgdma_read_write(file, (char __user *)io.buffer, io.len, &io.ep_addr, 2);
}
		...
		unsigned long arg)
{
	...
	case IOCTL_XDMA_APERTURE_W:
		...
		break;
	case IOCTL_XDMA_P2P_W:
		rv = ioctl_do_p2p_dma(file, arg);
		break;
	default:
