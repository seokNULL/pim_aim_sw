...
ssize_t p2p_write_from_buffer(char *fname, int fd, char *buffer, uint64_t size,
			uint64_t base)
{
	ssize_t rc = 0;
	uint64_t count = 0;
	...
	while (count < size) {
        ...
        io.buffer = (unsigned long)buf;
        io.len = bytes;
        io.ep_addr = rc;

        // clock_gettime(CLOCK_MONOTONIC, &start_FPGA2FPGA);

        rc = ioctl(fd, IOCTL_XDMA_P2P_W, &io);
        ...
        // printf("XDMA FPGA --> FPGA execution time %llu nanoseconds\n", (long long unsigned int) diff_FPGA2FPGA);
