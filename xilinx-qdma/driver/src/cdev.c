...
#ifdef NEWTON
static ssize_t cdev_gen_read_write_replace(struct file *file,
		struct qdma_mm_packet *mm_packet);
#endif // #ifdef NEWTON
...
#ifdef NEWTON
	struct qdma_mm_packet *mm_packet;
	uint8_t i = 0, chunk_size = 0, is_write = 0;
	int rv = 0;
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *) xcdev->xcb->xpdev->dev_hndl;
	unsigned long qhndl;
#endif
...
#ifdef NEWTON
		break;
	case QDMA_CDEV_IOCTL_PRESET:
		mm_packet = (struct qdma_mm_packet *) arg;
		for (i = 0; i < 1; i++) {
			mm_packet = &((struct qdma_mm_packet *) arg)[i];
			is_write = mm_packet->is_write;
			qhndl = is_write ? xcdev->h2c_qhndl : xcdev->c2h_qhndl;
			rv = qdma_descq_context_setup_replace(
				qdma_device_get_descq_by_id(xdev, qhndl, NULL, 0, 0),
				(unsigned long) mm_packet->mm_desc);
			if (rv < 0) {
		...
		return 0;
		break;
	case QDMA_CDEV_IOCTL_CMDS:
		mm_packet = (struct qdma_mm_packet *) arg;
		chunk_size = mm_packet[0].chunk_size;
		is_write = mm_packet->is_write;
		qhndl = is_write ? xcdev->h2c_qhndl : xcdev->c2h_qhndl;
		rv = qdma_descq_context_setup_replace(
				qdma_device_get_descq_by_id(xdev, qhndl, NULL, 0, 0),
		...
		for (i = 0; i < chunk_size; i++) {
			mm_packet = &((struct qdma_mm_packet *) arg)[i];
			rv = cdev_gen_read_write_replace(file, mm_packet);
			if (rv < 0) {
				pr_err("failed to clear the context, rv= %d", rv);
				return -EINVAL;
			}
...
#ifdef NEWTON
static int map_kernel_buf_to_sgl(struct qdma_io_cb *iocb, bool write)
{
	...
	/*asign page into iocb*/
	for (i = 0; i < pages_nr; i++) {
		if(is_vmalloc_addr(buf)){
			iocb->pages[i] = vmalloc_to_page((void *)(buf + PAGE_SIZE*i));
			pr_debug("va[%lx]->page[%lx], pa[%lx]\n", (unsigned long)(buf + PAGE_SIZE*i), (unsigned long)(iocb->pages[i]), (unsigned long)page_to_phys(iocb->pages[i]));
		}
		else
			iocb->pages[i] = virt_to_page((uint64_t)(buf + PAGE_SIZE*i));
	}
	...
	for (i = 0; i < pages_nr; i++, sg++) {
		...
		sg->offset = offset;
		sg->len = nbytes;
		sg->dma_addr = page_to_phys(iocb->pages[i]);
		sg->dma_addr += sg->offset;
		pr_debug("%s: dma addr:%llx", __func__, sg->dma_addr);
...
#ifdef NEWTON
static ssize_t cdev_gen_read_write(struct file *file, char *buf,
		size_t count, loff_t *pos, bool write)
#else
static ssize_t cdev_gen_read_write(struct file *file, char __user *buf,
		size_t count, loff_t *pos, bool write)
#endif
{
	...
	pr_debug("%s, priv 0x%lx: buf 0x%lx, size:%llu, pos %llu, W %d.\n",
		xcdev->name, qhndl, (unsigned long)buf, (u64)count, (u64)*pos,
		write);
...
#endif
	if (rv < 0) {
		printk("err return map kernel buf to sgl : %d", rv);
		return rv;
	}
...
#ifdef NEWTON
static ssize_t cdev_gen_read_write_replace(struct file *file, struct qdma_mm_packet *mm_packet)
{
	struct qdma_cdev *xcdev = (struct qdma_cdev *)file->private_data;
	struct qdma_io_cb iocb;
	struct qdma_request *req = &iocb.req;
	bool write = (bool) mm_packet->is_write;
	size_t length = mm_packet->len_mm_desc;
	ssize_t res = 0;
	unsigned long qhndl;

	if (!xcdev) {
		pr_err("file 0x%p, xcdev NULL, 0x%p,%llu, W %d.\n",
			file, mm_packet, (u64)length, write);
		return -EINVAL;
	}

	if (!xcdev->fp_rw) {
		pr_err("file 0x%p, %s, NO rw, 0x%p,%llu, W %d.\n",
			file, xcdev->name, mm_packet, (u64)length, write);
		return -EINVAL;
	}

	qhndl = write ? xcdev->h2c_qhndl : xcdev->c2h_qhndl;

	pr_debug("%s, priv 0x%lx: buf 0x%p,%llu, W %d.\n",
		xcdev->name, qhndl,  mm_packet, (u64)length, write);

	/*iocb setting*/
	memset(&iocb, 0, sizeof(struct qdma_io_cb));
	iocb.buf = mm_packet;
	iocb.pages_nr = length;
	iocb.is_kernel_memory = 1;
	...
	qdma_request_submit_replace(xcdev->xcb->xpdev->dev_hndl, qhndl, req);
	iocb_release(&iocb);
...
#endif // #ifdef NEWTON
#ifdef NEWTON
static ssize_t cdev_gen_write(struct file *file, const char *buf,
...
{
#else
static ssize_t cdev_gen_read(struct file *file, const char *buf,
				size_t count, loff_t *pos)
#endif
				...
				unsigned long count, loff_t pos)
{
	...
	for (i = 0; i < count; i++) {
		...
		rv = map_kernel_buf_to_sgl(&(caio->qiocb[i]), true);
		if (rv < 0)
			break;
						...
						unsigned long count, loff_t pos)
{
	...
	for (i = 0; i < count; i++) {
		...
		caio->qiocb[i].buf = io[i].iov_base;
		caio->qiocb[i].len = io[i].iov_len;
		rv = map_kernel_buf_to_sgl(&(caio->qiocb[i]), false);
		if (rv < 0)
			break;
...
{
	const struct iovec *iov = io->iov;
	pr_debug("w io->iov:%p, io[0].iov_base:%llx, io->nr_segs:%lx, io->ki_pos:%llx", io->iov, (uint64_t)iov->iov_base, io->nr_segs, iocb->ki_pos );
	return cdev_gen_write(iocb->ki_filp, iov->iov_base, iov[0].iov_len, &iocb->ki_pos);
...
{
	const struct iovec *iov = io->iov;
	pr_debug("r io->iov:%p, io[0].iov_base:%llx, io->nr_segs:%lx, io->ki_pos:%llx", io->iov,  (uint64_t)iov->iov_base, io->nr_segs, iocb->ki_pos );
	return cdev_gen_read(iocb->ki_filp, iov->iov_base, iov[0].iov_len, &iocb->ki_pos);
...
#ifdef NEWTON
static char *qdma_devnode(struct device *dev, umode_t *mode)
{
    if (mode)
        *mode = 0666;
    return NULL;
...
#ifdef NEWTON
	qdma_class->devnode = qdma_devnode;
#endif
