...
void qdma_update_request_replace(void *q_hndl, struct qdma_request *req,
			unsigned int num_desc)
{
	struct qdma_descq *descq = (struct qdma_descq *)q_hndl;
	struct qdma_sgt_req_cb *cb = qdma_req_cb_get(req);

	if (cb->desc_nr >= req->sgcnt) {
...
#ifdef NEWTON
ssize_t descq_mm_proc_request_replace(struct qdma_descq *descq)
{
	int rv = 0;
	...
	while (!list_empty(&descq->work_list)) {
		...
		if (cb->desc_nr >= req->sgcnt) {
			qdma_update_request_replace(descq, req, req->sgcnt);
			continue;
		}

		qdma_update_request_replace(descq, req, req->sgcnt);
		descq->pidx = (descq->pidx + req->sgcnt) %(descq->conf.rngsz - 1);
		cb->desc_nr += req->sgcnt;
		pr_debug("descq(%s), mm_desc(0x%x/0x%x), pidx(%u)\n",
			descq->conf.name, req->sgcnt, descq->avail, descq->pidx);
	}

	if (desc_written) {
		descq->pend_list_empty = 0;
		if (descq->pidx >= descq->conf.rngsz)
			descq->pidx = descq->pidx %(descq->conf.rngsz);
