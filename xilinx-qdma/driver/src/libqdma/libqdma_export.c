...
#ifdef NEWTON
static int make_sw_context_replace(struct qdma_descq *descq,
			   struct qdma_descq_sw_ctxt *sw_ctxt)
{
	...
	if ((descq->xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
		...
		sw_ctxt->vec = (descq->intr_id - descq->xdev->dvec_start_idx) + (descq->xdev->func_id * QDMA_NUM_DATA_VEC_FOR_INTR_CXT);
...
int qdma_descq_context_clear_replace(struct xlnx_dma_dev *xdev, unsigned int qid_hw,
...
int qdma_descq_context_program_replace(struct xlnx_dma_dev *xdev, unsigned int qid_hw,
			bool st, u8 type, struct qdma_descq_context *context)
{
	int rv;

	/* always clear first */
	rv = qdma_descq_context_clear_replace(xdev, qid_hw, st, type, 1);
	if (rv < 0) {
		return xdev->hw.qdma_get_error_code(rv);
	}
 ...
 * descriptor Queue
 */
static inline int get_desc_size_replace(struct qdma_descq *descq)
{
...
/*****************************************************************************/
/**
 * repalce qdma_mm_desc's PA & VA (actually va isn't used)
 *
 * @param descqq    struct qdma_descq * actually, not void * Queue Descriptor
 * @param kva		qdma_mm_desc virtual address, generated from LibAiM
 *
 * @returns		0: success <0: error
 *****************************************************************************/
int qdma_descq_context_setup_replace(void *descqq, unsigned long kva)
{
	struct qdma_descq_context context;
	struct qdma_descq *descq = (struct qdma_descq *) descqq;
	int rv = 0;

	if ((unsigned long) descq->desc != kva) {
		/* qdma_descq_prog_hw replace*/
		memset(&context, 0, sizeof(context));
		descq->desc = (u8 *) kva;
		descq->desc_bus = virt_to_phys((void *)kva);
		descq->desc_cmpl_status	= descq->desc + descq->conf.rngsz * get_desc_size_replace(descq);
		//descq->conf.rngsz = 0xffffffff; -> it doesn't work

		if (descq->conf.q_type != Q_CMPT) {
			make_sw_context_replace(descq, &context.sw_ctxt);
		}
		rv = qdma_descq_context_program_replace(descq->xdev, descq->qidx_hw,
					descq->conf.st, descq->conf.q_type, &context);

		if (unlikely(rv < 0)) {
			printk("%s: Failed to qdma_descq_context_program_replace\n",
					descq->conf.name);
			return -EFAULT;
		}
		...
		if (unlikely(rv < 0)) {
			printk("%s: Failed to update pidx\n",
					descq->conf.name);
			return -EINVAL;
		}
	}
...
/*****************************************************************************/
/**
 * qdma_request_submit_replace() - submit a scatter-gather list of data for dma
 ...
 * @return	<0: error
 *****************************************************************************/
ssize_t qdma_request_submit_replace(unsigned long dev_hndl, unsigned long id,
			struct qdma_request *req)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	struct qdma_descq *descq;
	...
	descq_mm_proc_request_replace(descq);
